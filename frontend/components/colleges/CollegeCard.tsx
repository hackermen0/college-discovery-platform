...
export function CollegeCard({ college, queryString = '' }: CollegeCardProps) {
  ...
  return (
    <div className="h-full bg-white rounded-lg shadow-md hover:shadow-lg transition-shadow duration-200 overflow-hidden flex flex-col border border-gray-100">
      {/* Header Section with Rating */}
      <div className="bg-gradient-to-r from-blue-50 to-indigo-50 p-4 border-b">
        <div className="flex justify-between items-start mb-2 gap-3">
          <Link href={href} className="font-semibold text-lg text-gray-900 line-clamp-2 flex-1 hover:text-blue-700 transition-colors">
            {college.name}
          </Link>
      ...
      <div className="bg-gray-50 px-4 py-3 border-t flex flex-col gap-2">
        <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
          <Link
            href={href}
            className="w-full text-center bg-blue-600 text-white font-semibold py-2 rounded-md hover:bg-blue-700 transition-colors text-sm"
          >
            View Details
          </Link>
          <CompareToggleButton collegeId={college.id} collegeName={college.name} compact />
        </div>
      </div>
    </div>
  );
